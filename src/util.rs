...
mod tests {
    ...
    fn test_strip_basedir_windows_mixed_slashes() {
        ...
        let output = super::strip_basedirs(input, std::slice::from_ref(&basedir));
        let expected = b"# 1 \"src\\main.c\"";
        // The basedirs must be normalized in the Config::from_env_and_file_configs
        assert_neq!(&*output, expected);
        let normalized_basedir = super::normalize_win_path(&basedir);
        let output = super::strip_basedirs(input, std::slice::from_ref(&normalized_basedir));
        assert_eq!(
            &*output, expected,
